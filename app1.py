...
def calculate_all_metrics(inputs):
    ...
    return {
        ...
        "annual_interest": annual_interest, "annual_depreciation": annual_depreciation,
        "annual_pbt": annual_pbt, "annual_tax": annual_tax, "annual_pat": annual_pat, "tax_rate_pct": tax_rate_pct,
...
def display_pnl(period_multiplier, period_name):
    ...
    c1.markdown(f"**Total Seed Input:** <br> {metrics['seed_input_mt'] * period_multiplier:.2f} MT", unsafe_allow_html=True)
    c2.markdown(f"**Oil Blend:** <br> {metrics['final_oil_blend_mt'] * period_multiplier:.2f} MT", unsafe_allow_html=True)
    c3.markdown(f"**Enhanced MoC:** <br> {metrics['enhanced_moc_mt'] * period_multiplier:.2f} MT", unsafe_allow_html=True)
    c4.markdown(f"**Total Revenue:** <br> ₹ {format_indian(metrics['daily_total_revenue'] * period_multiplier)}", unsafe_allow_html=True)
    st.markdown("---")
    
    st.markdown(f"##### Full Margin Analysis ({period_name})")
    c1, c2, c3 = st.columns(3)
    c1.markdown(f"**Gross Margin (GM):**<br> {format_indian(metrics['daily_gm'] * period_multiplier)}", unsafe_allow_html=True)
    c2.markdown(f"**Contribution Margin (CM):**<br> {format_indian(metrics['daily_cm'] * period_multiplier)}", unsafe_allow_html=True)
    c3.markdown(f"**EBITDA:**<br> {format_indian(metrics['daily_ebitda'] * period_multiplier)}", unsafe_allow_html=True)
    ...
    c1.markdown(f"**Depreciation:**<br> {format_indian(depreciation_for_period)}", unsafe_allow_html=True)
    c2.markdown(f"**Interest:**<br> {format_indian(interest_for_period)}", unsafe_allow_html=True)
    ...
    c3.markdown(f"**Profit Before Tax (PBT):**<br> {format_indian(pbt)}", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.metric("ROCE (PAT Basis)", f"{metrics['roce_pat']:.2f}%", help="Annualized PAT / (Capex + Total WC)")
        st.metric("ROCE (EBITDA Basis)", f"{metrics['roce_ebitda']:.2f}%", help="Annualized EBITDA / (Capex + Total WC)")
    with c2:
...
with wc_col:
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric("Total Inventory", f"₹ {metrics['total_inventory']/1e7:.2f} Cr")
    c2.metric("Total Debtors", f"₹ {metrics['total_debtors']/1e7:.2f} Cr")
    c3.metric("Total Creditors", f"₹ {metrics['total_creditors']/1e7:.2f} Cr")
    st.metric("Total Working Capital", f"₹ {metrics['total_wc']/1e7:.2f} Cr", delta_color="off")
    st.metric("Capex", f"₹ {metrics['capex']/1e7:.2f} Cr", delta_color="off")
with savings_col:
...
with st.expander("ℹ️ Click here to see key calculation logic"):
    ...
    - **Interest Calculation:** Annual interest is calculated using two separate rates for precision.
      ...
      - `Interest on Main Capital = (Capex + Remaining WC) * Main Financing Rate`
      - `Total Annual Interest` is the sum of these two components.
    - **PBT & PAT:** Calculated for each period.
      - `PBT = EBITDA - Depreciation - Interest`
      - `PAT = PBT - Tax (calculated only if PBT is positive)`
